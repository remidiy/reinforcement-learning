import gym
import itertools
import matplotlib
import numpy as np
import sys
import sklearn.pipeline
import sklearn.preprocessing
import warnings
warnings.filterwarnings('ignore')

if "../" not in sys.path:
  sys.path.append("../") 

from lib import plotting
from sklearn.linear_model import SGDRegressor
from sklearn.kernel_approximation import RBFSampler

matplotlib.style.use('ggplot')


env = gym.envs.make("MountainCar-v0")

observation_examples = np.array([env.observation_space.sample() for x in range(10000)])
scaler = sklearn.preprocessing.StandardScaler()
scaler.fit(observation_examples)

featurizer = sklearn.pipeline.FeatureUnion([
		("rbf1", RBFSampler(gamma=5.0, n_components=100)),
		("rbf2", RBFSampler(gamma=2.0, n_components=100)),
		("rbf3", RBFSampler(gamma=1.0, n_components=100)),
		("rbf4", RBFSampler(gamma=0.5, n_components=100))
		])
featurizer.fit(scaler.transform(observation_examples))

class Estimator():
	def __init__(self):
		self.models = []
		for _ in range(env.action_space.n):
			model = SGDRegressor(learning_rate="constant")
			model.partial_fit([self.featurize_state(env.reset())], [0])
			self.models.append(model)

	def featurize_state(self, state):
		scaled = scaler.transform([state])
		featurized = featurizer.transform(scaled)
		return featurized[0]

	def predict(self, state, action=None):
		if action is not None:
			value = self.models[action].predict([self.featurize_state(state)])[0]
			return value
		else:
			values = np.array([model.predict([self.featurize_state(state)])[0] for model in self.models])
			return values

	def update(self, state, action, target):
		self.models[action].partial_fit([self.featurize_state(state)], [target])

def epsilon_greedy_policy(estimator, epsilon, num_actions):
	def policy(observation):
		actions = np.ones(num_actions, dtype=float) * epsilon / num_actions
		best_action = np.argmax(estimator.predict(observation))
		actions[best_action] += 1.0 - epsilon
		return actions

	return policy



def q_learning(env, estimator, num_episodes, discount_factor=1.0, epsilon=0.1, epsilon_decay=1.0):
	
	episode_rewards=np.zeros(num_episodes)
	episode_lengths=np.zeros(num_episodes)

	for episode in range(num_episodes):

		policy = epsilon_greedy_policy(estimator, epsilon * epsilon_decay**episode, env.action_space.n)
		last_reward = episode_rewards[episode - 1]
		print('\repisode {}/{} reward: ({})'.format(episode + 1, num_episodes, last_reward), end="")
		sys.stdout.flush()

		state = env.reset()

		while True:
			action_dist = policy(state)
			action = np.random.choice(np.arange(env.action_space.n), p=action_dist)
			next_state, reward, done, _ = env.step(action)
			q_values = estimator.predict(next_state)
			td_target = reward + discount_factor * np.max(q_values)
			estimator.update(state, action, td_target)
			if done:
				break
			
			state = next_state
			episode_lengths[episode] += 1
			episode_rewards[episode] += reward

	stats = plotting.EpisodeStats(episode_lengths=episode_lengths, episode_rewards=episode_rewards)    
		
	return stats

estimator = Estimator()

stats = q_learning(env, estimator, 100, epsilon=0.0)

plotting.plot_cost_to_go_mountain_car(env, estimator)
plotting.plot_episode_stats(stats, smoothing_window=25)